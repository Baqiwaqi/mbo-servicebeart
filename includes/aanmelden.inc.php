...
if(isset($_POST["submit"])){
     ...
     if (empty($voornaam) || empty($achternaam) || empty($adres) ||
         ...
         header("location: ../aanmelden.php? error=emptyfields&voornaam=".$voornaam."&mail=".$email);
         exit();
     }
     ...
     elseif (!preg_match("/^[a_zA-Z0-9]*$/", $voornaam)) {
         header("location: ../aanmelden.php? error=invalidvoornaam&mail".$email);
         exit();
     }
     elseif (!preg_match("/^[0-9]{4}\s[A-Z]{2}$/", $postcode)) {
         header("location: ../aanmelden.php? error=invalidpostcode&voornaam=".$voornaam."&mail=".$email);
         exit();
     }
     elseif ($password !== $passwordHerhaling) {
         header("location: ../aanmelden.php? error=passwordCheck&voornaam=".$voornaam."&mail=".$email);
         exit();
     }
     else {
         ...
         if(!mysqli_stmt_prepare($stmt, $sql)) {
             header("Location ../aanmelden.php?error=sqlerror");
             exit();
         }
         else {
            ...
            if ($resultCheck > 0) {
                header("Location ../aanmelden.php?error=emailtaken");
                exit();
            }
            else {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_execute($stmt);
                    mysqli_stmt_store_result($stmt);
                    header("Location: ../inloggen.php?signup=succes");
                    exit();
                }
